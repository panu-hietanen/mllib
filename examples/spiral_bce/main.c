...
int main()
{
	...
	for (i32 epoch = 0; epoch < epochs; ++epoch)
	{
		...
		for (i32 it = 0; it < batches; ++it)
		{
			...
			Tensor *loss = graph_sigmoid_bce(arena_t, out, t_batch);
	...
	Tensor* h2 = graph_relu(arena_t, graph_add(arena_t, graph_matmul(arena_t, h1, w2), b2));
	Tensor* out = graph_add(arena_t, graph_matmul(arena_t, h2, w3), b3);
	Tensor* loss = graph_sigmoid_bce(arena_t, out, target);

	Tensor* weights[6] = { w1, w2, w3, b1, b2, b3 };
	data_save_tensors(weights, sizeof(weights) / sizeof(weights[0]), "../../../data/spiral_bce_weights.csv");

	Tensor* plotting[2] = { x, out };
	data_save_tensors(plotting, sizeof(plotting) / sizeof(plotting[0]), "../../../data/spiral_bce_data.csv");
